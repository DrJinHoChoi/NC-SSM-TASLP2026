...
\midrule
NM-Matched-DualPCEN & 7,402 & 7.2\,KB & 95.1\% & 12.8 \\
...
\multirow{4}{*}{Factory}
  ...
  & BC-ResNet-1 (7.5K)  & \textbf{95.3} & \textbf{64.5} & \textbf{74.8} & \textbf{83.5} & \textbf{89.0} & \textbf{91.9} & 92.9 & 93.9 \\
  & NM-Matched (7.4K)   & 95.1 & 61.1 & 70.6 & 82.3 & 88.5 & 91.4 & \textbf{93.0} & 93.8 \\
  & NM-Tiny (5.0K)      & 93.7 & 58.6 & 68.9 & 79.0 & 85.8 & 89.0 & 91.1 & 91.8 \\
\midrule
\multirow{4}{*}{White}
  ...
  & NM-Tiny (5.0K)      & 93.6 & 37.6 & 58.3 & 74.7 & 83.6 & 88.4 & 91.0 & 92.3 \\
\midrule
\multirow{4}{*}{Babble}
  ...
  & NM-Tiny (5.0K)      & 93.8 & 70.6 & 77.7 & 85.1 & 89.3 & 91.6 & 92.3 & 93.0 \\
\midrule
\multirow{4}{*}{Street}
  & DS-CNN-S (23.8K)    & 96.8 & 61.1 & 69.4 & 81.5 & 88.9 & 92.8 & 95.0 & 95.9 \\
  & BC-ResNet-1 (7.5K)  & \textbf{95.3} & \textbf{60.7} & \textbf{65.1} & \textbf{77.7} & \textbf{84.8} & \textbf{90.1} & 93.0 & 94.1 \\
  & NM-Matched (7.4K)   & 95.1 & 57.6 & 64.2 & 73.9 & 83.8 & 88.4 & \textbf{92.4} & \textbf{94.2} \\
  & NM-Tiny (5.0K)      & 93.8 & 58.2 & 64.6 & 71.7 & 83.3 & 87.3 & 91.0 & 92.4 \\
\midrule
\multirow{4}{*}{Pink}
  & DS-CNN-S (23.8K)    & 96.8 & 61.2 & 66.4 & 82.7 & 90.6 & 93.8 & 95.1 & 95.9 \\
  & BC-ResNet-1 (7.5K)  & \textbf{95.3} & \textbf{58.9} & \textbf{66.0} & \textbf{79.7} & \textbf{88.4} & 91.8 & \textbf{93.8} & \textbf{94.5} \\
  & NM-Matched (7.4K)   & 95.2 & 23.6 & 52.4 & 76.9 & 87.3 & \textbf{91.7} & 93.0 & 94.4 \\
  & NM-Tiny (5.0K)      & 93.8 & 28.3 & 56.7 & 75.4 & 84.8 & 89.4 & 91.9 & 93.2 \\
\bottomrule
\end{tabular}
\end{table*}
...
Calibration profiles (Table~\ref{tab:calibration}) are selected based on the estimated SNR during silence periods, enabling noise-level-aware adaptation at inference time without retraining.
...
The SNR-adaptive bypass mechanism (Section~\ref{sec:ss_bypass}) applies spectral subtraction only when beneficial: the bypass gate $\alpha$ approaches 1 at high SNR (preserving clean signals) and 0 at low SNR (enabling noise removal).
Table~\ref{tab:ss_bypass} presents the SS+Bypass results at $-15$\,dB across all noise types.

\begin{table}[t]
\centering
\caption{SS+Bypass improvement at $-15$\,dB SNR (\%p). Zero additional parameters. NanoMamba benefits far more than CNN baselines.}
\label{tab:ss_bypass}
\small
\begin{tabular}{lccccc}
\toprule
\textbf{Model} & \textbf{Fact.} & \textbf{White} & \textbf{Bab.} & \textbf{Street} & \textbf{Pink} \\
\midrule
NanoMamba   & +3.6 & \textbf{+23.8} & $-$0.1 & +1.7 & \textbf{+29.2} \\
...
(1)~\textbf{Stationary noise:} NanoMamba achieves +23.8\,\%p (white) and +29.2\,\%p (pink) at $-15$\,dB, while CNN baselines improve by only 0.5--3.7\,\%p.
The DualPCEN routing effectively integrates the SS-enhanced signal by directing it through the stationary-noise expert (Expert~2).
(2)~\textbf{Non-stationary noise:} All models show negligible change for babble ($-$0.1\,\%p), confirming that the bypass gate correctly passes through the original signal when SS would introduce artifacts.
(3)~\textbf{Clean preservation:} Clean accuracy is perfectly preserved across all models ($\pm$0\,\%p), validating the SNR-adaptive bypass design.

At 0\,dB, CNN baselines suffer notable degradation (BC-ResNet-1: $-$7.9\,\%p on white), whereas NanoMamba shows minimal impact ($-$0.5\,\%p).
This asymmetry demonstrates that NanoMamba's DualPCEN front-end is inherently more compatible with classical enhancement than fixed-normalization CNN architectures.

Notably, with SS+Bypass, NanoMamba-Tiny (4,957 params) surpasses BC-ResNet-1 (7,464 params) in extreme-SNR average accuracy on 3 of 5 noise types: white (+5.2\,\%p), street (+3.0\,\%p), and pink (+0.7\,\%p)---despite having 1.5$\times$ fewer parameters.


\subsection{Parameter-Matched Comparison}
\label{subsec:param_matched}

To enable a \emph{fair} comparison at equal model capacity, we scale NanoMamba to 7,402 parameters (NM-Matched-DualPCEN: $d_{\text{model}}=21$, $d_{\text{state}}=5$), matching BC-ResNet-1's 7,464 parameters within 0.8\%.

At this matched capacity, NanoMamba achieves \textbf{95.1\%} clean accuracy---only 0.2\,\%p below BC-ResNet-1 (95.3\%)---while requiring \textbf{6.8$\times$ fewer MACs} (0.68M vs 4.62M), \textbf{6.8$\times$ lower latency} on Cortex-M7 (1.42\,ms vs 9.63\,ms), and \textbf{3.2$\times$ less RAM} (31.7\,KB vs 102\,KB).

Table~\ref{tab:noise_main} shows that BC-ResNet-1 maintains an advantage at extreme noise ($-$15\,dB), particularly for broadband noise (white: 58.6\% vs 31.1\%, pink: 58.9\% vs 23.6\%).
However, at moderate-to-high SNR ($\geq$10\,dB), NanoMamba-Matched matches or exceeds BC-ResNet-1 on several noise types (e.g., factory 10\,dB: 93.0\% vs 92.9\%, street 15\,dB: 94.2\% vs 94.1\%).

This demonstrates that while SSM architectures trade broadband extreme-noise performance for dramatically superior computational efficiency, they achieve near-parity in the operating conditions most relevant to practical deployment ($\geq$0\,dB).
On CR2032 coin-cell batteries, this translates to \textbf{212 days} of continuous operation for NanoMamba-Matched versus 31 days for BC-ResNet-1---a 6.8$\times$ advantage that is decisive for battery-powered devices.
...
The parameter-matched comparison (Section~\ref{subsec:param_matched}) confirms this: at equal 7.4K parameters, NanoMamba achieves 95.1\% clean accuracy ($-$0.2\,\%p vs BC-ResNet-1) while requiring 6.8$\times$ fewer MACs---demonstrating that SSM architectures convert parameters to accuracy more efficiently than CNNs for sequential tasks like KWS.
...
The parameter-matched comparison (Section~\ref{subsec:param_matched}) demonstrates near-parity with BC-ResNet-1 at 7.4K parameters; scaling to $\sim$24K remains future work.
Several directions address remaining limitations:
(1)~\emph{Larger-scale comparison}: scaling NanoMamba to $\sim$24K parameters for fair comparison with DS-CNN-S;
...
At matched parameter count ($\sim$7.4K), NanoMamba achieves 95.1\% clean accuracy ($-$0.2\,\%p vs BC-ResNet-1) while requiring 6.8$\times$ fewer MACs and enabling 6.8$\times$ longer battery life on coin-cell-powered devices.

These results establish that ultra-compact SSMs with structural noise adaptation represent a principled approach to noise-robust edge KWS, achieving CNN-competitive accuracy at dramatically lower computational cost---uniquely suited for always-on, battery-powered deployments where efficiency is as critical as accuracy.
