...
\author{Anonymous ACCV submission\\Paper ID \cvprPaperID}
...
For video, we extend NC-Conv with a \textcolor{tempcolor}{bidirectional temporal NC-SSM} with per-frame quality gating, enabling clean-frame context to recover $\mathbf{+35.2\%}$ accuracy on the darkest tunnel frame while preserving clean-frame accuracy within $1\%$.
...
\item \textbf{Temporal NC-SSM}: Extension to video with per-frame quality-conditioned temporal dynamics for illumination-varying sequences (Sections~\ref{sec:temporal_theory} and~\ref{sec:temporal_exp}).
