...
\subsection{Learned $\sigma$ Gate Visualization}
...
% ===== ROW 1: Spatial σ maps =====

% (a) Normal
\node[font=\footnotesize\bfseries, anchor=west] at (0, 4.2) {(a) Normal conditions};
\foreach \r in {0,...,3} {
  \foreach \c in {0,...,7} {
    \pgfmathsetmacro{\s}{0.75 + rand*0.18}
    \pgfmathsetmacro{\g}{min(95, max(5, \s*100))}
    \fill[green!\g!red, rounded corners=0.5pt]
      (\c*0.48, 3.6-\r*0.48) rectangle (\c*0.48+0.42, 3.6-\r*0.48-0.42);
  }
}
\node[font=\scriptsize, color=black!70] at (1.9, 1.5) {$\sigma \approx 0.85$ (dynamic path)};

% (b) Dark tunnel
\node[font=\footnotesize\bfseries, anchor=west] at (5, 4.2) {(b) Dark tunnel};
\foreach \r in {0,...,3} {
  \foreach \c in {0,...,7} {
    \pgfmathsetmacro{\base}{0.04 + \r*0.03}
    \pgfmathsetmacro{\road}{(\c > 2 && \c < 6 && \r > 1) ? 0.15 : 0}
    \pgfmathsetmacro{\s}{\base + \road + rand*0.04}
    \pgfmathsetmacro{\g}{min(95, max(5, \s*100))}
    \fill[green!\g!red, rounded corners=0.5pt]
      (5+\c*0.48, 3.6-\r*0.48) rectangle (5+\c*0.48+0.42, 3.6-\r*0.48-0.42);
  }
}
\node[font=\scriptsize, color=black!70] at (6.9, 1.5) {$\sigma \approx 0.1$ (static path)};

% Legend
\fill[red!80, rounded corners=1pt] (4, 4.15) rectangle (4.3, 3.95);
\node[font=\scriptsize, anchor=west] at (4.35, 4.05) {$\sigma{=}0$};
\fill[green!80, rounded corners=1pt] (4, 3.75) rectangle (4.3, 3.55);
\node[font=\scriptsize, anchor=west] at (4.35, 3.65) {$\sigma{=}1$};

% ===== ROW 2: Temporal gate + Context transfer =====
\draw[black!15, line width=0.5pt] (0, 0.9) -- (8.8, 0.9);

% (c) Temporal gate bars
\node[font=\footnotesize\bfseries, anchor=west] at (0, 0.5) {(c) Temporal quality gate $g_t$};
\foreach \i/\g in {0/0.004, 1/0.016, 2/0.067, 3/0.101, 4/0.092, 5/0.040, 6/0.026, 7/0.004} {
  \pgfmathsetmacro{\h}{\g/0.12*2.4}
  \pgfmathsetmacro{\isHigh}{\g > 0.05 ? 1 : 0}
  \ifnum\isHigh=1
    \fill[orange!80, rounded corners=1pt]
      (\i*0.52, -0.1) rectangle (\i*0.52+0.38, -0.1-\h);
  \else
    \fill[green!60!black, rounded corners=1pt]
      (\i*0.52, -0.1) rectangle (\i*0.52+0.38, -0.1-\h);
  \fi
  \node[font=\tiny, color=black!70] at (\i*0.52+0.19, 0.1) {$f_\i$};
  \node[font=\tiny] at (\i*0.52+0.19, -0.1-\h-0.15) {\g};
}
\node[font=\scriptsize, color=black!70, anchor=north west] at (0, -2.4) {clean$\,\to\,g{\approx}0$};
\node[font=\scriptsize, color=orange!80, anchor=north west] at (1.6, -2.4) {degraded$\,\to\,g{\uparrow}$};

% (d) Context transfer
\node[font=\footnotesize\bfseries, anchor=west] at (5, 0.5) {(d) Context transfer};
\node[fill=green!25, rounded corners=3pt, minimum width=1.3cm, minimum height=0.55cm,
  font=\scriptsize, inner sep=2pt] (n0) at (5.4, -0.5) {$f_0$ clean};
\node[fill=red!30, rounded corners=3pt, minimum width=1.3cm, minimum height=0.55cm,
  font=\scriptsize\bfseries, inner sep=2pt] (n3) at (6.9, -1.5) {$f_3$ dark};
\node[fill=green!25, rounded corners=3pt, minimum width=1.3cm, minimum height=0.55cm,
  font=\scriptsize, inner sep=2pt] (n7) at (8.4, -0.5) {$f_7$ clean};
\draw[-{Stealth[length=2mm]}, green!60!black, line width=0.8pt]
  (n0.south) -- (n3.north west) node[midway, above, sloped, font=\tiny] {forward};
\draw[-{Stealth[length=2mm]}, tempcolor!80, line width=0.8pt]
  (n7.south) -- (n3.north east) node[midway, above, sloped, font=\tiny] {backward};
\node[font=\scriptsize, color=black!70, text width=3.2cm, align=center, anchor=north]
  at (6.9, -2.1) {52.6\%\,$\to$\,\textbf{87.8\%} (+35.2\%)};

\end{tikzpicture}%
}
\caption{\textbf{Learned $\sigma$ gate visualization.} (a)~Normal: $\sigma{\approx}0.85$, dynamic path. (b)~Dark tunnel: $\sigma{\approx}0.1$, static path. (c)~Temporal gate peaks at darkest frames. (d)~Bidirectional context: $f_0$, $f_7$ help $f_3$. All learned without quality labels.}
\label{fig:sigma_viz}
\end{figure}

Figure~\ref{fig:sigma_viz} visualizes the learned $\sigma$ gates.
The spatial $\sigma$ map (a,b) shows automatic adaptation: under normal conditions, $\sigma \approx 0.85$ activates the dynamic path everywhere; under dark tunnel conditions, $\sigma$ drops to $\sim$0.1, routing features through the robust static path.
The temporal gate (c) tracks the degradation profile exactly, peaking at the darkest frame without any explicit quality supervision.
This demonstrates that the NC framework discovers input quality structure purely through task loss.
