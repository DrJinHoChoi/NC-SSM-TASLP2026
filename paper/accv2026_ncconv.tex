...
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[
  cell/.style={minimum width=0.55cm, minimum height=0.55cm, font=\tiny, inner sep=0pt},
]
% --- Spatial σ: Normal ---
\node[font=\scriptsize\bfseries] at (2.2, 1.0) {(a) Spatial $\sigma$ --- Normal};
\foreach \r in {0,...,5} {
  \foreach \c in {0,...,7} {
    \pgfmathsetmacro{\s}{0.78 + rand*0.12}
    \pgfmathsetmacro{\g}{min(100, \s*100)}
    \node[cell, fill=green!\g!red] at (\c*0.6, -\r*0.6) {};
  }
}
\node[font=\tiny, color=gray] at (2.2, -3.5) {$\sigma \approx 0.85$ everywhere (all dynamic)};

% --- Spatial σ: Dark/Tunnel ---
\node[font=\scriptsize\bfseries] at (8.2, 1.0) {(b) Spatial $\sigma$ --- Dark Tunnel};
\foreach \r in {0,...,5} {
  \foreach \c in {0,...,7} {
    \pgfmathsetmacro{\base}{0.05 + \r*0.03}
    \pgfmathsetmacro{\road}{(\c > 2 && \c < 6 && \r > 3) ? 0.15 : 0}
    \pgfmathsetmacro{\s}{\base + \road + rand*0.05}
    \pgfmathsetmacro{\g}{min(100, \s*100)}
    \node[cell, fill=green!\g!red] at (6+\c*0.6, -\r*0.6) {};
  }
}
\node[font=\tiny, color=gray] at (8.2, -3.5) {$\sigma \approx 0.1$ (mostly static/robust)};

% --- Temporal gate ---
\node[font=\scriptsize\bfseries] at (2.2, -4.8) {(c) Temporal quality gate $g_t$};
\foreach \i/\g/\lab in {0/0.004/clean, 1/0.016/appr., 2/0.067/entry, 3/0.101/inside, 4/0.092/ins.+n, 5/0.040/exit, 6/0.026/recov., 7/0.004/clean} {
  \pgfmathsetmacro{\h}{\g/0.12*2.0}
  \pgfmathsetmacro{\col}{\g > 0.05 ? "orange" : "green!70!black"}
  \fill[\col, opacity=0.7] (\i*0.7, -5.2) rectangle (\i*0.7+0.5, -5.2-\h);
  \node[font=\tiny, anchor=north] at (\i*0.7+0.25, -5.2-\h-0.05) {\g};
  \node[font=\tiny, color=gray, anchor=south] at (\i*0.7+0.25, -5.0) {$f_\i$};
}
\node[font=\tiny, color=gray] at (2.8, -7.5) {clean $\to g{\approx}0$ (no correction) \quad degraded $\to g{\uparrow}$ (temporal correction)};

% --- Temporal gate: what it does ---
\node[font=\scriptsize\bfseries] at (8.2, -4.8) {(d) Context transfer};
\draw[-{Stealth}, green!60!black, thick] (6.5, -5.8) -- (8.0, -6.5) node[midway, above, font=\tiny, sloped] {clean};
\draw[-{Stealth}, green!60!black, thick] (10.0, -5.8) -- (8.5, -6.5) node[midway, above, font=\tiny, sloped] {clean};
\node[fill=red!30, rounded corners=3pt, font=\scriptsize, minimum width=1.2cm, minimum height=0.6cm] at (8.2, -6.7) {$f_3$ dark};
\node[font=\tiny, color=gray] at (8.2, -7.5) {$f_0$(clean) and $f_7$(clean) both help $f_3$(dark)};

% Color legend
\node[fill=red!80, minimum width=0.4cm, minimum height=0.3cm] at (5.5, 1.0) {};
\node[font=\tiny, anchor=west] at (5.8, 1.0) {$\sigma{=}0$};
\node[fill=green!80, minimum width=0.4cm, minimum height=0.3cm] at (7.0, 1.0) {};
\node[font=\tiny, anchor=west] at (7.3, 1.0) {$\sigma{=}1$};

\end{tikzpicture}%
}
\caption{\textbf{Learned $\sigma$ gate visualization.} (a)~Normal: $\sigma \approx 0.85$ everywhere, dynamic path dominates. (b)~Dark tunnel: $\sigma \approx 0.1$, static path activated for noise robustness. Road center slightly higher (lane markings visible). (c)~Temporal quality gate: peaks at darkest frames ($g = 0.101$ at $f_3$), near zero for clean frames. (d)~Bidirectional context transfer: both clean past ($f_0$) and clean future ($f_7$) help degraded $f_3$. All gates learned without explicit quality labels.}
\label{fig:sigma_viz}
\end{figure}
